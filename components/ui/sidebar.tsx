...
>(
    (
        ...
        const state = open ? "expanded" : "collapsed" // Definimos state aquí
...
>(({ className, onClick,variant = "sidebar", ...props  }, ref) => {
    ...
    const showPanelLeft = variant === "navbar" && (state === "collapsed" || (isMobile && !openMobile))
    const icon = showPanelLeft ? <PanelLeft /> : <ChevronsLeft />
...
>(({ className, ...props }, ref) => {
    const { isDragging, setIsDragging } = useSidebar()
    ...
    return (
        <button
            ...
            className={cn(
                "absolute inset-y-0 right-0 z-20 w-1 transition-colors hover:bg-blue-600/50",
